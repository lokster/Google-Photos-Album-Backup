...
finds matching photos in a directory, and restores their timestamps.

Modes available:
  --exif (-e):       Update EXIF metadata for photos WITHOUT existing timestamps
  --filesystem (-f): Update filesystem modification/access times for ALL photos
  --verify (-v):     Compare existing EXIF timestamps against JSON (no modifications)
...
def get_exif_dates(exiftool, filepath):
    """Get existing EXIF date fields from a file using exiftool."""
    try:
...
def set_exif_dates(exiftool, filepath, dt, ts, dry_run=False):
    """Set EXIF date fields using exiftool, including sub-second precision."""
    ...
    def build_args(fpath):
        return [
            ...
            f"-FileModifyDate={exif_datetime}",
    ...
    if dry_run:
        print(f"  [DRY RUN] Would set EXIF dates")
        return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix photo timestamps based on exported JSON from Google Photos Album Backup"
    )
    parser.add_argument("json_file", help="Path to the exported JSON file")
    parser.add_argument("photo_directory", help="Directory containing photos to process")
    parser.add_argument(
        "--dry-run", "-n",
        action="store_true",
    ...
    parser.add_argument(
        "--verify", "-v",
        action="store_true",
        help="Verify existing EXIF timestamps against JSON (no modifications)"
    )
    ...
    # Progress indicator
    progress = Progress(len(photos), "Processing")

    try:
        for filename, timestamps in sorted(photos.items()):
            progress.update()

            filepath = find_file_in_directory(filename, args.photo_directory)
    ...
    print("\n" + "=" * 60)
    print("Summary:")
    print(f"  Photos in JSON:        {len(photos)}")
    print(f"  Found in directory:    {stats['found']}")
    print(f"  Not found:             {stats['not_found']}")
    if args.verify:
        print(f"  With EXIF metadata:    {stats['has_metadata']}")
        ...
        print(f"  EXIF updated:          {stats['exif_updated']}")
    if args.filesystem:
